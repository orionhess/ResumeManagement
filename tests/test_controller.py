...
def test_get_sections():
    controller: Controller = Controller(get_items())
    sections = controller.get_sections()
    assert sections == get_items()
...
def test_get_sections_active():
    controller: Controller = Controller(get_items())
    active_sections = controller.get_sections_active()
    assert len(active_sections) == 2
    assert active_sections[0]["text"] == "section 1"
    assert active_sections[1]["text"] == "section 3"
